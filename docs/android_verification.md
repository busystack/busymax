...
| PASSED | Repository-wide unit and widget suite | `flutter test --concurrency=1 --reporter compact` | 2,275 tests passed and 10 explicitly environment-gated tests were skipped in the current correction working tree. The skipped live-integration cases are not counted as passes. |
| PASSED | Android platform test suite | `flutter test test/platform/android --concurrency=1` | 69 current Android unit/widget tests passed in one serialized run. This includes the scenario-specific rows below; it is not counted again as an additional 69 scenarios. |
...
| PASSED | Android adaptive widget matrix | `flutter test test/platform/android/android_adaptive_app_test.dart --concurrency=1` | 31 widget tests passed. The empty shell matrix covers the required sizes/themes/RTL. Populated flows explicitly select Day, Week, Month, and Year, exercise all-day/date-only overflow at compact and wide Week widths, submit and inspect an invitation response, and render Month/Year at 320x500 and 200% text. Long task details retain their full notes and scroll to the Edit action at ordinary-phone, 700x320 landscape, and 200% text constraints. System Back is exercised independently for every named event/task text field. Stateful secure-storage tests restore a new event's saved destination and time even when the editor reopens on another date, and round-trip recurring scope and advanced task recurrence. Google partial free/busy failures and omitted recipient results render Unknown while an explicit successful empty result renders Free. Dirty task save-and-move, cancel, and failure paths inspect real database/recovery state. Settings routes, Apple credential-prompt lifecycle, Google calendar rename/color/delete sync requests, and feedback failure/retry identity are exercised. |
| PASSED | Android notification policy | `flutter test test/platform/android/android_notification_policy_test.dart --concurrency=1` | 8 helper/contract tests passed: stable IDs, quiet-hours eligibility, pending/displayed retention predicates, privacy/exact/content fingerprinting, explicit Open UI action, and payload parsing. |
| PASSED | Android notification reconciliation | `flutter test test/platform/android/android_notification_reconciliation_test.dart --concurrency=1` | 8 service tests passed against database rows and a stateful pending/active platform fake: displayed retention, stale generation cancellation, Dismiss, overdue inexact pending retention, Snooze, quiet-hours deferral, content-only pending replacement, and overdue daily-summary privacy/precision replacement. |
...
| PASSED | Calendar and tasks — automated | Virtual Agenda, time-axis Day/Week with bounded accessible all-day overflow, scrollable civil-date Month with selected-day agenda, and Year miniatures are implemented. Current populated Android fixtures verify overnight intersection, source/list hiding, Week selection, invitation controls, all-day overflow, short/large-text Month/Year, and full scrollable details at phone, short-landscape, and enlarged-text constraints. Editor tests cover read-only controls, account-qualified destinations, every named text field's system-Back guard, versioned recovery identity across changed creation defaults, recurrence/scope, and dirty task moves. Shared provider payload tests remain a separate evidence layer. |
...
| PASSED | Logical viewport rendering | The empty-shell matrix covers required logical sizes, themes, scales, RTL, and every primary destination. The populated matrix separately covers Day/Week data, short 320x500/200% Month/Year, and long scrollable details at 412x915, 700x320, and 200% text without framework exceptions. This is logical widget rendering, not a device screenshot. |
