...
| Verification time | `2026-09-14T13:40:36-07:00` (`America/Vancouver`) |
...
| PASSED | Formatting | bundled `dart format --output=none --set-exit-if-changed .` | 710 files unchanged by the check |
...
| PASSED | Unit and widget suite | `flutter test --reporter compact` | 2,204 runnable tests passed; 10 live/environment tests skipped and are not reported as passes; [flutter-test.log](../build/android/flutter-test.log) |
| PASSED | Android adaptive widget matrix | `flutter test test/platform/android/android_adaptive_app_test.dart` | Schedule, Tasks and Settings exercised at 360x800, 412x915, 800x1280, landscape 915x412 and split 320x500, text scales 1.0/1.3/2.0, light/dark and RTL; no overflow; compact/wide navigation, labels and 48 dp controls checked |
| PASSED | Android notification policy | `flutter test test/platform/android/android_notification_policy_test.dart` | Stable IDs, collision handling, quiet-hours behavior and generation-bearing action payloads |
| PASSED | Android platform channel contract | `flutter test test/platform/android/android_platform_channel_test.dart` | Provider availability, account-specific silent Microsoft request and content-URI behavior |
...
| PASSED | Native plugin unit tests | `android/gradlew --project-dir android :busymax_android_platform:testDebugUnitTest` | JUnit report under `build/busymax_android_platform/reports/tests/testDebugUnitTest/`; unknown methods return `notImplemented` |
...
|---|---:|---|
| `build/app/outputs/flutter-apk/app-release.apk` | 84,506,244 bytes | `775fb6f3753b1e07b262453147030bad4dea988a8eb806b6738dbd12a91bff81` |
| `build/app/outputs/bundle/release/app-release.aab` | 80,651,652 bytes | `0a862e8a0d3bff9ec8e63a56145c2c4cb0b402d98d38761f00f2e6e2d37fcf49` |
...
| PASSED | Google — implementation/contract | Native `AuthorizationClient`, explicit selection, exact BusyMax/native binding, silent token path, removal isolation, Play-services availability, Calendar/Tasks repository reuse, and no Android desktop OAuth imports are implemented and contract-tested. |
...
| PASSED | Microsoft — implementation/contract | MSAL multiple-account browser flow, exact native-account binding, account-specific silent acquisition, interaction-required propagation and removal isolation are implemented and channel-tested. |
...
| PASSED | Nextcloud — automated | Existing DAV engines, Login Flow v2 state, capabilities, pending operations, conflict behavior, collection administration and Android contextual-LAN result mapping pass shared/unit coverage; UI consumes concrete collection capabilities. |
...
| PASSED | Calendar and tasks — automated | Agenda/day/week/month/year, all-day/timed/date-only items, recurrence/exceptions, due/undated/completed tasks, fields, hierarchy, read-only capabilities, source/account identity and empty/busy/error states are exercised by shared and Android widget tests. |
...
| PASSED | Database and preferences — automated | Schema migration through version 18, foreground/background visibility, atomic settings writes, account removal, coordination release and account resurrection protections are covered by database, settings and sync tests. |
...
| PASSED | Notifications — automated | Durable mapping, horizon/cap, exact/inexact policy, generation/stale-action rejection, edit/cancel/dedup, Open/Snooze/Dismiss persistence, quiet/private/summary logic and independent reconciliation gate pass deterministic tests. Future scheduling is not labeled delivery. |
...
| PASSED | Files and links — automated | ICS preview-before-import, no silent mutation, bounded and sanitized content-URI reads, SAF create/tree operations, malformed/oversized/cancel/duplicate activation handling, geo/browser fallback and missing-handler feedback are implemented and contract-tested. |
