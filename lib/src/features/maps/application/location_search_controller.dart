...
final class LocationSearchController extends ChangeNotifier {
  LocationSearchController(this.client, {this.debounce = const Duration(milliseconds: 300)});
  ...
  void search(String text, {required String language, bool composing = false, bool submit = false, bool autocomplete = true}) {
    clear();
    if (composing || text.trim().isEmpty || (!submit && text.trim().runes.length < 3)) return;
    ...
    Future<void> run() async {
      ...
      notifyListeners();
      try {
        final found = await client.search(text, language: language, autocomplete: autocomplete, cancellation: request);
        if (_disposed || generation != _generation) return;
        results = found;
        status = found.isEmpty ? LocationLookupStatus.empty : LocationLookupStatus.results;
    ...
    if (submit) { unawaited(run()); } else { _timer = Timer(debounce, run); }
  }
  void clear() {
    _generation++;
    _timer?.cancel();
  ...
  @override
  void dispose() { _disposed = true; clear(); super.dispose(); }
}
