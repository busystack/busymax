...
final class GeographicPoint {
  ...
  factory GeographicPoint({required double latitude, required double longitude}) {
  ...
  static GeographicPoint? tryParse({required Object? latitude, required Object? longitude}) {
    ...
    if (lat == null || lon == null || !lat.isFinite || !lon.isFinite || lat.abs() > 90 || lon.abs() > 180) return null;
    return GeographicPoint._(latitude: lat, longitude: lon);
  }
  static GeographicPoint? fromJson(Object? value) => value is Map ? tryParse(latitude: value['latitude'], longitude: value['longitude']) : null;
  static GeographicPoint? fromIcal(String? value) {
    final parts = value?.split(';');
    return parts?.length == 2 ? tryParse(latitude: parts![0], longitude: parts[1]) : null;
  }
  ...
  @override
  bool operator ==(Object other) => other is GeographicPoint && other.latitude == latitude && other.longitude == longitude;
  @override
